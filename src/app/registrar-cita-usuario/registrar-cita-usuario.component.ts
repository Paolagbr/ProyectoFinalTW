...
@Component({
  ...
  styleUrls: ['./registrar-cita-usuario.component.css']
})
export class RegistrarCitaUsuarioComponent implements OnInit {
  ...
  nuevoUsuario(): void {
    const emailUsuario = this.auth.currentUser?.email || '';
    ...
    this.usuariosService.addPlace(nuevaCita).then(() => { // Guardar info en BD
      this.horasOcupadas = [];
      const servicioNombre = this.grupos.find(g => g.id === this.userINFO.grupo)?.nomServicio || 'Desconocido';
      ...
      // Enviar correo usando llamada directa al backend
      this.enviarCorreo({
        name: this.userINFO.name,
        fechaCita: this.userINFO.fechaCita,
        hora: this.userINFO.hora,
  ...
  enviarCorreo(datosCorreo: any) {
    return this.http.post(`${this.apiUrl}/enviar-correo`, datosCorreo);
  }
...
//     private usuariosService: UsuariosService, 
//     private correoService: CorreoService,  private auth: Auth, private cd: ChangeDetectorRef) { }
